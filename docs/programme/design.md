...
- 领域驱动设计(DDD)
  - 系统自然生长的轨迹：伴随着业务的增长不断演进，期间通过不断的重构、测试驱动应付各种混乱的问题
    - 测试驱动:可以确保功能不丢失不破坏
    - 重构是克服系统大杂烩问题的主力，但是重构很容易的抽取一些通用的逻辑，但是很难给它定义一个业务上的含义，只能赋予一个技术维度的描述，对于这段抽取的通用逻辑对新人不友好、破坏代码的可读性，这样的重构就成了自娱自乐，代码继续腐败，重新重构……无休止的循环
